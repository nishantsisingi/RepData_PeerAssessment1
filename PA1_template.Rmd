...
```{r, message=FALSE}
ggplot(daily_steps) + geom_histogram(aes(x=tot))
```
...
```{r}
ggplot(interval_steps) + geom_line(aes(x=interval,y=avg))
```
...
```{r, message=FALSE}
ggplot(new_total_steps) + geom_histogram(aes(x=tot))
```
...
ggplot(aes(x=interval,y=avg)) + geom_line()+ geom_smooth() + facet_wrap(~wtype,ncol = 1,nrow = 2) 
```
